...
#constant that determines the membrane potential decay time which sets the rise and decay time of the
#PSP
memPotDecayT = 28

#constant that sets the spiking threshold of the neurons
threshold = 0
...
class AsyncSN:
	...
	#to make connections have different number of terminals just send a list instead of integer for 
	#terminals parameter
	def __init__(self, connections, terminals, inhibN):
		...
		for s in range(connections):
				self.synapses[s] = Connection(terminals, connections)
	...
	def lifFunctionDer(self, time, nType):
		...
		div = 1 - (float(time)/memPotDecayT)
		return div  * math.exp(div) / memPotDecayT
